...
function renderClaudePanel(): string {
  ...
  const needsKey = auth?.needs_api_key !== false;
...
const AI_PROVIDERS: { id: AiProvider; label: string; note: string }[] = [
  { id: "claude", label: "Claude", note: "console.anthropic.com → API keys" },
  { id: "gemini", label: "Gemini", note: "aistudio.google.com → Get API key" },
  { id: "ollama", label: "Ollama", note: "Runs on your machine — no API key" },
];
...
async function refreshClaudeAuth() {
  try {
    ...
    state.claudeAuth = {
      provider: aiProvider(),
      label: AI_PROVIDERS.find((p) => p.id === aiProvider())?.label ?? "AI",
      model: "",
      base_url: "",
      needs_api_key: true,
      signed_in: false,
